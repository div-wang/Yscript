...
参考了百度spec的代码规范，结合三九养生堂实际开发习惯。更好的规范了JavaScript代码风格的一直性，让javascript开发效率更高更便于维护维护。
...
##### [强制] 使用 `4` 个空格tab做为一个缩进层级，不允许使用 `2` 个空格 `tab` 字符。
...
##### [建议] 对于基本类型 {string}, {number}, {boolean}，首字母必须小写。
...
##### [建议] 使用包装方式扩展类成员时， 必须通过 `@lends` 进行重新指向。
...
##### [建议] 类的属性或方法等成员信息使用 `@public` / `@protected` / `@private` 中的任意一个，指明可访问性。
...
##### [强制] AMD/CMD 模块使用 `@module` 或 `@exports` 标识。
...
##### [强制] 对于已使用 `@module` 标识为 AMD/CMD模块 的引用，在 `namepaths` 中必须增加 `module:` 作前缀。
...
##### [强制] 书写复杂函数或者类时，变量必须 `即用即声明`，不得在函数或其它形式的代码块起始位置统一声明所有变量。
...
通常在 JavaScript 中声明的对象，属性命名是使用 驼峰 命名法，用 `.` 来访问更清晰简洁。部分特殊的属性(比如来自后端的JSON)，可能采用不寻常的命名方式，可以通过 `[expr]` 方式访问。
...
Sea.js开发文档(http://seajs.org/docs/)
